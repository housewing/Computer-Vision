...
int main(int argc, char** argv[])
{
	...
	cv::SVD svd(mat8x9, SVD::FULL_UV); // solve SVD matrix
	Mat v = svd.vt.t();
	cout << "v size " << v.size() << ", col " << v.cols << ", row " << v.rows << endl;
	Mat tmp = v.col(8); // get the last column value
	cout << "lats column of v " << tmp << endl;

	//put the last column value into H matrix 
	...
	//Mat mat3x1 = (Mat_<float>(3, 1) << B[3].x, B[3].y, 1); // test point
	...
	vector<Mat> videoMat; // store all of frames into vector
	for (;;)
	{
		Mat frame;
	...
	//imshow("clone", src);
	vector<Point> Apt; // store all points which are indise A vector (4 points)
	vector<Point> Bpt; // store all points which are indise B vector (4 points)
	for (int y = 0; y < src.rows; y++)
	{
		for (int x = 0; x < src.cols; x++)
		{
			if (pointPolygonTest(Mat(B), Point(x, y), true) > 0) // measure whether a point inside B vector(4 points)
			{
				Bpt.push_back(Point(x, y));
			}
			else if (pointPolygonTest(Mat(A), Point(x, y), true) > 0) // measure whether a point inside A vector(4 points)
			{
	...
	vector<Mat> homographyMat; // store new image get from H matrix
	for (unsigned int j = 0; j < videoMat.size(); j++)
	{
		...
		{
			int tx = Bpt[i].x; // x pixel point of right monitor
			int ty = Bpt[i].y; // y pixel point of right monitor
			...
			uchar* outData = copyMat.ptr<uchar>(ty);
			outData[3 * tx] = videoMat[j].at<Vec3b>(tmpY, tmpX)[0]; // exchange b pixel value
			outData[3 * tx + 1] = videoMat[j].at<Vec3b>(tmpY, tmpX)[1]; // exchange g pixel value
			outData[3 * tx + 2] = videoMat[j].at<Vec3b>(tmpY, tmpX)[2]; // exchange r pixel value
		}

		for (unsigned int i = 0; i < Apt.size(); i++)
		{
			int tx = Apt[i].x; // x pixel point of left monitor
			int ty = Apt[i].y; // y pixel point of left monitor
	...
	for (unsigned int i = 0; i < homographyMat.size(); i++)
	{
		...
		oVideoWriter.write(homographyMat[i]); // write video
	}
	cout << "done" << endl;
